""" MOS data file reader to look at individual  calibration files, then use this
to predict the [VOC] of the next calibration file.
"""
import pandas as pd
import numpy as np
...
#for f in filenames:
	...
	offset=T1-T2
	data.TheTime+=offset
	# Work out the [VOC] from the mfc flowrates.
	...
	# Change the variables to plot a calibration time.
	mosdf = pd.concat([data_concat.dt,data_concat.MOS1,data_concat.MOS2,data_concat.VOC_mr],axis=1)
	mosdf = mosdf.dropna()
	x_var = ['VOC_mr']
	x_train = mosdf[x_var]
	y_train = mosdf.MOS1
...
# Plot the background.
	...
	mos1ax.plot(mosdf.VOC_mr, mosdf.MOS1,color='b',marker='o')
	mos1ax.plot(mosdf.VOC_mr, MOS1_pred)
	mos1ax.set_ylabel("MOS1 (V)")
	mos1ax.set_xlabel("[VOC] (ppb)")

	mos1ax = mosfig.add_subplot(2,1,2)
	mos1ax.plot(mosdf.VOC_mr,mosdf.MOS2,color='r',marker='o')
	mos1ax.plot(mosdf.VOC_mr,MOS2_pred)
	mos1ax.set_ylabel("MOS2 (V)")
	mos1ax.set_ylabel("[VOC] (ppb)")
...
calibration_file = ['d20160201_02']
a1,a2,b1,b2 = cal_pred(calibration_file)
...
bin1 = stats.binned_statistic(data_concat.VOC_mr, MOS1_cor, statistic='mean', bins=1000, range=(0,100))
bin2 = stats.binned_statistic(data_concat.VOC_mr, MOS2_cor, statistic='mean', bins=1000, range=(0,100))
bin3 = stats.binned_statistic(data_concat.VOC_mr, data_concat.VOC_mr, statistic='mean', bins=1000, range=(0,100))
bin4 = stats.binned_statistic(data_concat.VOC_mr, MOS1_cor, statistic=stddev, bins=1000, range=(0,100))
bin5 = stats.binned_statistic(data_concat.VOC_mr, MOS2_cor, statistic=stddev, bins=1000, range=(0,100))
bin6 = stats.binned_statistic(data_concat.VOC_mr, data_concat.VOC_mr, statistic=stddev, bins=1000, range=(0,100))
