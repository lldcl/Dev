...
path = '/Users/ks826/Google Drive/Data_Analysis/Raw_data_files/201509/'

filename = 'd20150918_05'
stat = 'Y'
print filename
...
isop_cyl = 13.277	#1000 for cylinder after 11/2015 #ppbv
mfchi_range = 100.	# 2000 for system  after 11/2015 #sccm
...
data.RH = (((data.RH/4.96)-0.16)/0.0062)/(1.0546-0.00216*data.Temp*10.)
...
# 		sfig.plot(df_xrange.index,pd.rolling_mean(df_xrange[p],100),color=c)
		ax2 = sfig.twinx()
# 		ax2.plot(data.index,df_xrange.RH, linewidth=3,color='k')
...
# 		plt.ylabel("RH / %")
	...
	print ' RH max', np.max(df_xrange.RH)
	print 'RH min', np.min(df_xrange.RH)
#######################################
...
rhax.plot(data.TheTime,data.RH,color='b',linewidth=3)
rhax.set_ylabel("RH %")

tmp = pd.DataFrame(data.RH.interpolate())
