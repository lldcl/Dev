...
ozone = plt.figure()
ax1 = ozone.add_subplot(4,1,1)
ax5 = ax1.twinx()
ax2 = ozone.add_subplot(4,1,2)
ax6 = ax2.twinx()
ax3 = ozone.add_subplot(4,1,3)
ax4 = ozone.add_subplot(4,1,4)
...
ax4.set_ylabel("RH (%)")
ax5.plot(data_concat.index, data_concat.O3*1000, color = "silver")
ax6.plot(data_concat.index, data_concat.O3*1000, color = "silver")
ax5.set_ylabel("Ozone (ppb)")
ax6.set_ylabel("Ozone (ppb)")
...
if (stat == 'Y'):
	...
	O3select = plt.figure("Plot the selected regions")
	ax1 = O3select.add_subplot(1,1,1)
	ax2 = O3select.add_subplot(2,1,2)
...
cal_pointsMOS1 = [np.mean(df_xrange1.MOS1), np.mean(df_xrange2.MOS1),np.mean(df_xrange3.MOS1)]
cal_pointsMOS2 = [np.mean(df_xrange1.MOS2), np.mean(df_xrange2.MOS2),np.mean(df_xrange3.MOS2)]
stdeMOS1 = [np.std(df_xrange1.MOS1), np.std(df_xrange2.MOS1),np.std(df_xrange3.MOS1)]
stdeMOS2 = [np.std(df_xrange1.MOS2), np.std(df_xrange2.MOS2),np.std(df_xrange3.MOS2)]
stdeO3 = [np.std(df_xrange1.O3), np.std(df_xrange2.O3),np.std(df_xrange3.O3)]

conc = [np.mean(df_xrange1.O3), np.mean(df_xrange2.O3),np.mean(df_xrange3.O3)]


# Plotting an ozone cal.
cal = plt.figure()
ax1 = cal.add_subplot(2,1,1)
ax2 = cal.add_subplot(2,1,2)
ax1.scatter(conc, cal_pointsMOS1, color = "forestgreen")
ax1.errorbar(conc, cal_pointsMOS1, xerr=stdeO3, yerr=stdeMOS1, lw=1, fmt='o', color="g")
ax1.set_xlabel("O3 (ppb)")
...
ax2.errorbar(conc, cal_pointsMOS2, xerr=stdeO3, yerr=stdeMOS2, lw=1, fmt='o', color="m")
ax2.set_xlabel("O3 (ppb)")
...
ax1.text((np.min(conc))*1.0000001, np.min(cal_pointsMOS1)-0.01 ,'y = ' +str(slope1)+'x +' +str(intercept1), style='italic', fontsize = 15) 
ax1.text((np.min(conc))*1.0000001, np.min(cal_pointsMOS1)-0.02, '$R^2 value$ = '+str(R21), style = 'italic', fontsize=15)
ax2.text((np.min(conc))*1.0000001, 1.5 ,'y = ' +str(slope2)+'x +' +str(intercept2), style='italic', fontsize = 15) 
ax2.text((np.min(conc))*1.0000001, 1.4, '$R^2 value$ = '+str(R22), style = 'italic', fontsize=15)
