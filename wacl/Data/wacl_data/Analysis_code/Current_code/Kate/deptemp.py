...
# Get rid of data with isoprene in it as just want to look at temperature variability
isopdrop = data_concat[data_concat.isop_mr > 0.01] 
isopdrop.Temp = isopdrop.Temp * 100

ax1 = temp_dep.add_subplot(2,1,1)
ax2 = temp_dep.add_subplot(2,1,2)
ax1.scatter(isopdrop.Temp, isopdrop.MOS1, linewidth=3, color="g")
ax2.scatter(isopdrop.Temp, isopdrop.MOS2, linewidth=3, color="b")
ax1.set_ylabel("MOS1")
ax1.set_xlabel("Temperature")
ax2.set_ylabel("MOS2")
ax2.set_xlabel("Temperature")
slope1, intercept1, R2value1, p_value1, st_err1 = stats.linregress(isopdrop.Temp, isopdrop.MOS1)
slope2, intercept2, R2value2, p_value2, st_err2 = stats.linregress(isopdrop.Temp, isopdrop.MOS2)
ax1.plot([np.min(isopdrop.Temp), np.max(isopdrop.Temp)], [(slope1*np.min(isopdrop.Temp))+intercept1, (slope1*np.max(isopdrop.Temp))+intercept1])
ax2.plot([np.min(isopdrop.Temp), np.max(isopdrop.Temp)], [(slope2*np.min(isopdrop.Temp))+intercept2, (slope2*np.max(isopdrop.Temp))+intercept2])
...
# Bin the data into 0.5C of temperature sections
bin1 = stats.binned_statistic(isopdrop.Temp, isopdrop.MOS1, statistic='mean', bins=10, range=(25,35))
bin2 = stats.binned_statistic(isopdrop.Temp, isopdrop.MOS2, statistic='mean', bins=10, range=(25,35))
bin3 = stats.binned_statistic(isopdrop.Temp, isopdrop.Temp, statistic='mean', bins=10, range=(25,35))
# Binning the standard deviation also
#bin4 = stats.binned_statistic(data_concat.Temp, data_concat.MOS1, statistic=stddev, bins=20, range=(25,35))
#bin5 = stats.binned_statistic(data_concat.Temp, data_concat.MOS2, statistic=stddev, bins=20, range=(25,35))
#bin6 = stats.binned_statistic(data_concat.Temp, data_concat.Temp, statistic=stddev, bins=20, range=(25,35))
# Turn into pandas to get rid of NaNs.
binMOS1 = pd.Series(bin1[0])
binMOS2 = pd.Series(bin2[0])
Temp = pd.Series(bin3[0])
...
binMOS1 = binMOS1.dropna()
binMOS2 = binMOS2.dropna()
Temp = Temp.dropna()
#st1 = st1.dropna()
#st2 = st2.dropna()
#stIs = stIs.dropna()

# Re plot the temperature variance.
temp_dep1 = plt.figure("Temperature variability with temperature binned into 0.5C increments")
# Get rid of data with isoprene in it as just want to look at temperature variability

ax1 = temp_dep1.add_subplot(2,1,1)
ax2 = temp_dep1.add_subplot(2,1,2)
ax1.scatter(Temp, binMOS1, linewidth=3, color="g")
ax2.scatter(Temp, binMOS2, linewidth=3, color="b")
ax1.set_ylabel("MOS1")
ax1.set_xlabel("Temperature")
ax2.set_ylabel("MOS2")
ax2.set_xlabel("Temperature")
slope1, intercept1, R2value1, p_value1, st_err1 = stats.linregress(Temp, binMOS1)
slope2, intercept2, R2value2, p_value2, st_err2 = stats.linregress(Temp, binMOS2)
...
"""
# Correct for overall background:
