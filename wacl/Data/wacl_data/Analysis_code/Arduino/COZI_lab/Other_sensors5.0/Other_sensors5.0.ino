...
// Setting addresses for the third eurocircuit board.
...
//Statistics header for the third eurocircuit boardss' sensors.
...
//Set the channel for the second eurocircuit board
  ...
  ads1115_4Bb.begin();
 //Set the channel for the third eurocircuit board
  tcaselect(6);
  ads1115_48c.setGain(GAIN_ONE);        // 2x gain   +/- 2.048V  1 bit = 1mV      0.0625mV
  ads1115_48c.begin();
  ads1115_49c.setGain(GAIN_ONE);        // 2x gain   +/- 2.048V  1 bit = 1mV      0.0625mV
  ads1115_49c.begin();
  ads1115_4Ac.setGain(GAIN_ONE);        // 2x gain   +/- 2.048V  1 bit = 1mV      0.0625mV
  ads1115_4Ac.begin();
  ads1115_4Bc.setGain(GAIN_ONE);        // 2x gain   +/- 2.048V  1 bit = 1mV      0.0625mV
  ads1115_4Bc.begin();
...
// Set the channel for the NO and CO sensors.
  ...
  ads1115_B.begin();
// Set the channel for the voltage and pressure channels. 
//  tcaselect(6);
...
//take sample loop reads each sensor voltage and adds to statistic instance
  ...
  tcaselect(6);
  MOS1c_stats.add(ads1115_4Bb.readADC_Differential_0_1());
  MOS2c_stats.add(ads1115_49b.readADC_Differential_2_3());
  MOS3c_stats.add(ads1115_4Ab.readADC_Differential_2_3());
  MOS4c_stats.add(ads1115_48b.readADC_Differential_2_3());
  MOS5c_stats.add(ads1115_4Ab.readADC_Differential_0_1());
  MOS6c_stats.add(ads1115_48b.readADC_Differential_0_1());
  MOS7c_stats.add(ads1115_4Bb.readADC_Differential_2_3());
  MOS8c_stats.add(ads1115_49b.readADC_Differential_0_1());
...
// Telling the data to be averaged and the standard deviation for the MOS to be calculated
    ...
    double MOS1c_Av = MOS1c_stats.average()*gainone_multiplier*1e-3;
    double MOS1c_stddev =  MOS1c_stats.pop_stdev()*gainone_multiplier*1e-3;    
    double MOS2c_Av = MOS2c_stats.average()*gainone_multiplier*1e-3;
    double MOS2c_stddev =  MOS2c_stats.pop_stdev()*gainone_multiplier*1e-3;
    double MOS3c_Av = MOS3c_stats.average()*gainone_multiplier*1e-3;
    double MOS3c_stddev =  MOS3c_stats.pop_stdev()*gainone_multiplier*1e-3;
    double MOS4c_Av = MOS4c_stats.average()*gainone_multiplier*1e-3;
    double MOS4c_stddev =  MOS4c_stats.pop_stdev()*gainone_multiplier*1e-3;
    double MOS5c_Av = MOS5c_stats.average()*gainone_multiplier*1e-3;
    double MOS5c_stddev =  MOS5c_stats.pop_stdev()*gainone_multiplier*1e-3;
    double MOS6c_Av = MOS6c_stats.average()*gainone_multiplier*1e-3;
    double MOS6c_stddev =  MOS6c_stats.pop_stdev()*gainone_multiplier*1e-3;
    double MOS7c_Av = MOS7c_stats.average()*gainone_multiplier*1e-3;
    double MOS7c_stddev =  MOS7c_stats.pop_stdev()*gainone_multiplier*1e-3;
    double MOS8c_Av = MOS8c_stats.average()*gainone_multiplier*1e-3;
    double MOS8c_stddev =  MOS8c_stats.pop_stdev()*gainone_multiplier*1e-3;
...
// Print and store the average. Must call each multiplexer address before printing information from that section.  
      ...
      Serial.print(",");
      Serial.print(LM65T1_stddev);
      Serial.print(",");
      Serial.print(SV_Av);
      Serial.print(",");
      Serial.print(SV_stddev);
      ...
      Serial.print(MOS1b_stddev,8);
      Serial.print(",");
      Serial.print(MOS2b_stddev,8);
      Serial.print(",");
      Serial.print(MOS3b_stddev,8);
      Serial.print(",");
      Serial.print(MOS4b_stddev,8);
      Serial.print(",");
      Serial.print(MOS5b_stddev,8);
      Serial.print(",");
      Serial.print(MOS6b_stddev,8);
      Serial.print(",");
      Serial.print(MOS7b_stddev,8);
      Serial.print(",");
      Serial.print(MOS8b_stddev,8);
      Serial.print(",");

      Serial.print(MOS1c_stddev,8);
      Serial.print(",");
      Serial.print(MOS2c_stddev,8);
      Serial.print(",");
      Serial.print(MOS3c_stddev,8);
      Serial.print(",");
      Serial.print(MOS4c_stddev,8);
      Serial.print(",");
      Serial.print(MOS5c_stddev,8);
      Serial.print(",");
      Serial.print(MOS6c_stddev,8);
      Serial.print(",");
      Serial.print(MOS7c_stddev,8);
      Serial.print(",");
      Serial.print(MOS8c_stddev,8);
      Serial.println(",");
