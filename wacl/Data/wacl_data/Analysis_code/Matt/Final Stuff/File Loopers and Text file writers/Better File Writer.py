...
path = '/Users/Pete/Google Drive/Pete/AlFHoNSo/Data_Analysis/RH_tests/Raw_data//201509'
...
for qwe in to_be_read:
    ...
    #SF = None #The code always trips up half way through, so I popped this in to see if it'll fix it.                
    dataL = pd.read_csv(filesl[qwe])                       
...
if there_are_new_values_to_be_added == 'Y':   
    ...
    if there_is_already_data_in_the_file == 'Y':
        ...
        Final_Values = pd.concat([data_already_in_cal_points,New_Values],axis = 0)
...
#        New_Values.to_csv(path_or_buf='C:\Users\mat_e_000\Google Drive\Bursary\Data_Analysis\\RH Cal Points.txt', sep=',')
    ...
    if there_is_already_slopes_in_the_file == 'Y':
        ...
        Final_Slopes = pd.concat([data_already_there_slopes,Slope_Intercepts],axis = 0)
