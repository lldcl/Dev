...
try:
    ...
    slope_string = ''
    for num in pids_on:
        slope_string = slope_string + '\tSlope %s\tSlope Error %s'%(str(num),str(num))
    ...
    Pid_String = ''
    for num in pids_on:
        Pid_String = Pid_String + '\tPid %s\tPid Error %s'%(str(num),str(num))
    ...
    try:
        ...
        if proceed.upper() == 'Y' or proceed.upper() == 'YES':
            ...
            for pkl in xrange(0,len(cal_df)):
                ...
                for num in pids_on:
                    Pid_voltage_string = Pid_voltage_string +'\t'+ str(cal_df.loc[pkl,'Pid%s Voltage'%str(num)])+'\t'+str(cal_df.loc[pkl,'Pid%s Stderr'%str(num)])
                
                calfile.write(filename+'\t'+str(dataL.TheTime[len(dataL.TheTime)/2])+'\t'+RH_mean+'\t'+str(cal_df.loc[pkl,'Isop'])+Pid_voltage_string+'\n')
                ...
                if humid_correct == 'Y': 
                    ...
                    for num in pids_on:
                        Pid_voltage_stringc = Pid_voltage_stringc +'\t'+ str(cal_dfc.loc[pkl,'Pid%s Voltage'%str(num)])+'\t'+str(cal_dfc.loc[pkl,'Pid%s Stderr'%str(num)])
                    
                    calfilec.write(filename+'\t'+str(dataL.TheTime[len(dataL.TheTime)/2])+'\t'+RH_mean+'\t'+str(cal_dfc.loc[pkl,'Isop'])+Pid_voltage_string+'\n')
        ...
        for num in pids_on: 
            slope_values_string = slope_values_string + '\t' + str(Values['Slope'][3]) + '\t' + str(Values['Slope Error'][3])
        ...
        if humid_correct == 'Y':
            ...
            for num in pids_on: 
                slope_values_stringc = slope_values_stringc + '\t' + str(Corrected_Values['Slope'][3]) + '\t' + str(Corrected_Values['Slope Error'][3])
            
            slopescorrectedfile.write(str(dataL.TheTime[len(dataL.TheTime)/2])+'\t'+RH_mean+slope_values_stringc+'\n')
          
          
        n = n+1
        if n == len(already_read_checker):
            print '\n\nAll Done! :D'
        print '%s down, %s to go'%(n,str(len(already_read_checker)-n)), '( ',np.around(float(n*100)/len(already_read_checker),1),'% )'   
         
    except 'bob': #If there is an error pop it here and it will not read that cal, it won't record that cal data though.
        print '\n\n--------------------\n\n'
