...
def plotter(pidxd,pidxdc):
    ...
    times = list_multisplicer(tim,S_dict[pid_number],F_dict[pid_number])
                                                            
    voltage_pid = list_multisplicer(pidxd,S_dict[pid_number],F_dict[pid_number])
            
    isop_chunks_data_pid = list_multisplicer(isop_mr,S_dict[pid_number],F_dict[pid_number])
    
    if humid_correct ==  'Y':
        
            timesc = list_multisplicer(tim,Sc_dict[pid_number],Fc_dict[pid_number])
                    
            isop_chunks_data_pidc = list_multisplicer(isop_mr,Sc_dict[pid_number],Fc_dict[pid_number])
    ...
    if humid_correct == 'Y':
        ...
        voltage_pidc = list_multisplicer(pidxdc,Sc_dict[pid_number],Fc_dict[pid_number])
...
# Takes Averages of the spliced lists
def chunk_avgerager(chunks):
        return [np.nanmean(i) for i in chunks]
#Takes Standard Deviations of the spliced lists
def chunk_stddever(chunks):
        return [np.nanstd(i) for i in chunks]
...
if proceed.upper() == 'Y' or proceed.upper() == 'YES':
    ...
    if (cal == 'Y'):
        ...
        if pid_3_on == 'Y':
            if humid_correct == 'Y':      
                pid3_chunksc = list_multisplicer_listsout(pid3dc,Sc_dict[3],Fc_dict[3])
                
                pid3_avgsc = chunk_avgerager(pid3_chunksc)
                pid3_stdsc = chunk_stddever(pid3_chunksc)

                
    
            
            pid3_chunks = list_multisplicer_listsout(pid3d,S_dict[3],F_dict[3])
        ...
        if pid_4_on =='Y':
            if humid_correct == 'Y':  
                pid4_chunksc = list_multisplicer_listsout(pid4dc,Sc_dict[4],Fc_dict[4])
            ...
            pid4_chunks = list_multisplicer_listsout(pid4d,S_dict[4],F_dict[4])
        ...
        if pid_5_on =='Y':
            if humid_correct == 'Y':
                pid5_chunksc = list_multisplicer_listsout(pid5dc,Sc_dict[5],Fc_dict[5])
                
                pid5_avgsc = [np.nanmean(i) for i in pid5_chunksc]
                pid5_stdsc = [np.nanstd(i) for i in pid5_chunksc] 
    
            
            pid5_chunks = list_multisplicer_listsout(pid5d,S_dict[5],F_dict[5])
