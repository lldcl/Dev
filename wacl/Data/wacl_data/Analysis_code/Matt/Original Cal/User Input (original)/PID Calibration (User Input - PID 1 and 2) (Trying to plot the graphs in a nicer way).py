...
if plots == 'Y':
    ...
    fig_messyone = plt.figure() # Creating figures for the best line fit graph and the messy one
    fig_slopeone = plt.figure()
    
    axes1_pid2 = fig_messyone.add_subplot(2,1,1) # Creating a set of axes for each PID in one figure for the raw data plot
    axes1_pid3 = fig_messyone.add_subplot(2,1,2)
    
    axes2_pid2 = fig_slopeone.add_subplot(2,1,1) # Creating a set of axes for each PID in one figure for the best fit line plot
    axes2_pid3 = fig_slopeone.add_subplot(2,1,2)
    
    pid2_avg = pd.rolling_mean(data1.pid1,100) #Taking the mean of the data very 3 seconds
    pid3_avg = pd.rolling_mean(data1.pid2,100)
    
    avg_plot_pid2 = axes1_pid2.plot(Time[tmp_pid2],pid2_avg[tmp_pid2]) #Plottng the rolling mean
    avg_plot_pid3 = axes1_pid3.plot(Time[tmp_pid2],pid2_avg[tmp_pid2])
    
    pid2 = axes1_pid2.plot(Time[tmp_pid2],data1.pid2[tmp_pid2], linewidth=3,color='r')
    pid3 = axes1_pid3.plot(Time[tmp_pid2],data1.pid2[tmp_pid2], linewidth=3,color='r')

    isop = axes1_pid3.plot(Time[tmp_flow],isop_mr[tmp_flow], linewidth=3,color='g')
...
if (plots == 'Y'):
	...
	ax1 = fig1.add_subplot(111)#ADDING A SET OF AXES
...
if (plots == 'Y'):
	...
	pid_10s = ax1.plot(Time[tmp_pid3],pid3_avg[tmp_pid3], linewidth=3,color='b')
