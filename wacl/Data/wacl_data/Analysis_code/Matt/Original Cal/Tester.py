...
if (cal == 'Y'):
    ...
    try:
        ...
        zero_time = 0 # how long the zero at the end will be
    
    
        mfc_setF = data1.index[mfc_set]
        if mfc_setF[-1]+zero_time < len(data1):
            ...
            print '\nERROR MESSAGE:'
            print '\n\nThe zero time at the end was a bit big, please lower it to ' + str(len(data1)-mfc_setF[-1] -1) + ' or below'
            print  '\n\n(Around line 152)\n\n'
        ...
        #check that the flows are the same at the start and end of the range over which you are averaging
            if (np.logical_and(data1.mfclo[mfc_setS[pt]]==data1.mfclo[mfc_setF[pt]],data1.mfchi[mfc_setS[pt]]==data1.mfchi[mfc_setF[pt]])):
                    ...
                    '''The Outliers bit isn't working and I don't know why'''
                    ...
                    '''
                    pid2dats = tp_pid2.tolist()
                    
                    for i in range(0,len(tp_pid2)-1):

                        if abs(pid2dats[i]) > m2+s2 or abs(pid2dats[i]) < m2 - s2:
                            bad_eggs.append(i)
                    
                    
                    data1.pid2 = data1.pid2.drop(data1.pid2.index[bad_eggs])'''
