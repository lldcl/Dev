...
std_resampled.RH1 = (((std_resampled.RH1/mean_resampled.VS)-0.16)/0.0062)/(1.0546-0.00216*std_resampled.Temp*100.)
...
dRHdt = pd.Series(np.absolute(mean_resampled.isop_mr.diff()),name='dRHdt')
...
# RH_cal_data,raw_cal_data = RH_cal_fn(path,'201511/d20151109_03','30S')
...
# for p in pids:
#    name = str('Corr_'+p)
#    tmp_ar = (mean_resampled[p]*1e3)-(RH_cal_data[p][0]*mean_resampled.RH)
...
rhax.plot(mean_resampled.index,mean_resampled.RH1,color='b',linewidth=3)
rhax.set_ylabel("RH %")
...
filtisop_cal_fits = pd.DataFrame(columns = pids, index = ['slope','slope_error','intercept','intercept_error'])
for p,c in zip(pids,colors):
# 	name= str('Corr_'+p)
	name = p
	fit_data = odr.RealData(x = mean_dRHdt_filt.isop_mr,y = mean_dRHdt_filt[name],sx = std_dRHdt_filt.isop_mr ,sy = std_dRHdt_filt[name]*1e3)
	fit = odr.ODR(fit_data,linear,[5e-5,0.05])
	fit_params = fit.run()
