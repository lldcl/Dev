...
for n,c in zip(voc1,colors):
    ...
    ltext  = leg.get_texts()  # all the text.Text instance in the legend
    plt.setp(ltext, fontsize='large')    # the legend text fontsize
    plt.ylabel("vocs1/MOSc*150", size=20)
    plt.xlabel("Time", size=20)	
VOCs1fig.show()


sub = 'vocs2'
voc2=[ 'C5H8+ (isoprene;NO+) (ppb)',	'C4H6O.NO+ (3-buten-2-one;NO+) (ppb)',	'NO+.C4H8O (butanone;NO+) (ppb)',	'C6H6+ (benzene;NO+) (ppb)',	'NO.C6H6+ (benzene;NO+) (ppb)',	'C7H8+ (toluene;NO+) (ppb)',	'C8H10+ (m-xylene;NO+) (ppb)',	'C9H12+ (1,2,4-trimethylbenzene;NO+) (ppb)',	'C4H6+ (1,3-butadiene;NO+) (ppb)', 'MOS1c_Av']
...
for n,c in zip(voc2,colors):
    ...
    ltext  = leg.get_texts()  # all the text.Text instance in the legend
    plt.setp(ltext, fontsize='large')    # the legend text fontsize
    plt.ylabel("vocs2/MOSc*150", size=20)
    plt.xlabel("Time", size=20)	
VOCs2fig.show()

sub = 'vocs3'
voc3=[ 'C8H17+ (octane;NO+) (ppb)',	'C10H21+ (decane;NO+) (ppb)',	'C2H5O+ (ethanol;NO+) (ppb)',	'C3H6O+ (acetone;O2+) (ppb)',	'C5H7+ (isoprene;O2+) (ppb)',	'C5H8+ (isoprene;O2+) (ppb)',	'C4H8O+ (butanone;O2+) (ppb)',	'C6H6+ (benzene;O2+) (ppb)',	'C7H8+ (toluene;O2+) (ppb)',	'C7H7+ (m-xylene;O2+) (ppb)', 'MOS1c_Av']
...
for n,c in zip(voc3,colors):
    ...
    ltext  = leg.get_texts()  # all the text.Text instance in the legend
    plt.setp(ltext, fontsize='large')    # the legend text fontsize
    plt.ylabel("vocs3/MOSc*150", size=20)
    plt.xlabel("Time", size=20)	
VOCs3fig.show()
...
for n,c in zip(voc4,colors):
    ...
    ltext  = leg.get_texts()  # all the text.Text instance in the legend
    plt.setp(ltext, fontsize='large')    # the legend text fontsize
    plt.ylabel("vocs4/MOSc*150", size=20)
    plt.xlabel("Time", size=20)	
plt.show()